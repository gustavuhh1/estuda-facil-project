...
class AnexoController {
    ...
    public ResponseEntity<Anexo> uploadTarefa(@PathVariable Long tarefaId, @RequestParam("file") MultipartFile file) throws IOException {
    ...
    public ResponseEntity<Anexo> uploadAviso(@PathVariable Long avisoId, @RequestParam("file") MultipartFile file) throws IOException {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.salvarAnexoParaAviso(avisoId, file));
    }

    @GetMapping("/tarefa/{tarefaId}")
    public ResponseEntity<List<Anexo>> listarTarefa(@PathVariable Long tarefaId) {
        return ResponseEntity.ok(service.listarAnexosPorTarefa(tarefaId));
    }

    @GetMapping("/aviso/{avisoId}")
    public ResponseEntity<List<Anexo>> listarAviso(@PathVariable Long avisoId) {
